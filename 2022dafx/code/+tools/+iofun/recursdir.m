function listfiles = recursdir(basedir,ext)
%RECURSDIR Recursive search in BASEDIR in subfolders for files of type .EXT
%
%   FILES = RECURSDIR(BASEDIR,EXT) search in BASEDIR and subfolders for
%   files of type EXT. BASEDIR must be a path to a valid folder. EXT must
%   be a valid file extension.
%
%   EXT can be preceded of the dot (.ext) or not (ext). RECURSDIR adds the
...
for idir = 1:ndir
    ...
    if ~listdir(idir).isdir && ~isempty(regexp(listdir(idir).name,['\.' ext],'match'))
    ...
    elseif listdir(idir).isdir && ~strcmp(listdir(idir).name,'.') && ~strcmp(listdir(idir).name,'..')
        ...
        % Recursive search
        OutfilesTemp = tools.iofun.recursdir(pname,ext);
