...
if nargin == 3 % tools.math.simps(x,y,dim)
    
    perm = [dim:max(ndims(y),dim) 1:dim-1];
    yp = permute(y,perm);
    [m,n] = size(yp);
    
elseif nargin==2 && isscalar(y) % tools.math.simps(y,dim)
...
else % tools.math.simps(y) or tools.math.simps(x,y)
...
z = sum(a0(1:2:end,:).*y(1:2:m-2,:) + a1(1:2:end,:).*y(2:2:m-1,:) + a2(1:2:end,:).*y(3:2:m,:),1);
