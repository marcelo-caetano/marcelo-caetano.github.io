function fig = mkfigspectropeakgram(plotdata,axeslim,axeslbl,figlayout)
%MKFIGSPECTROPEAKGRAM Make the figure with the spectral peaks over the
%spectrogram.
%   FIG = MKFIGSPECTROPEAKGRAM(PLOTDATA,AXESLIM,AXESLBL,FIGLAYOUT)
%
...
%   PLOTDATA.TIMESPEC time vector in (s) with NFRAME rows.
%   PLOTDATA.FREQSPEC frequency vector in (Hz) with NFREQ rows.
%   PLOTDATA.TIMEPEAK NFREQ x NFRAME matrix with time in (s) for all peaks
%   PLOTDATA.FREQPEAK NFREQ x NFRAME matrix with peak frequency in (Hz) for all time frames
%
...
%
%   See also MKFIGWAV, MKFIGSPEC, MKFIGSPECTROGRAM, MKFIGPEAKGRAM, MKFIGPARTTRACK
...
[fig,figaxes,specgram,peakgram,nrglbl,timelbl,freqlbl,titlelbl] = ...
    tools.plot.plotspectropeakgram(plotdata.logmagspec,plotdata.timespec,plotdata.freqspec,...
    plotdata.timepeak,plotdata.freqpeak);
...
if isfield(figlayout,'font')
    ...
    % Customize energy axis font name
...
if isfield(figlayout,'axesfs')
    ...
    % Customize energy axis font size
...
% Set marker type
set(peakgram,'Marker',figlayout.mtype);

% Set marker size
set(peakgram,'MarkerSize',figlayout.msize);

% Set line style
set(peakgram,'LineStyle',figlayout.linsty);
