...
for isound = startsnd:endsnd
    ...
    [fft_frame,center_frame,nsample,nframe,nchannel,dc] = tools.stft.stft(wav,framelen,hop,nfft,...
        winflag,causalflag{cf},normflag,zphflag);
    
    plot_data.logmagspec = tools.fft2.fft2log_mag_spec(fft_frame(:,:,1),nfft,logflag{lmsf},nrgflag,nanflag);
    
    plot_data.timespec = center_frame/fs;
    
    plot_data.freqspec = tools.plot.mkfreq(nfft,fs,'pos',true)/1000;
    ...
    plot_data.timepeak = repmat(center_frame/fs,[1,npartial])';
    
    %plot_data.freqpeak = cat(2,frequency{:})/1000;
    plot_data.freqpeak = frequency/1000;
    
    % Time limits
    axes_lim.tlim = [plot_data.timespec(1) plot_data.timespec(end)];
    ...
    % Make figure
    fig = tools.plot.mkfigspectropeakgram(plot_data,axes_lim,axes_lbl,fig_layout);
    
    % Save figure
    saveas(fig,fullfile(savimg,[fname '_spectropeakgram']),'pdf');
